...
function getUserDisplayEle(user) {
    return `<div class="user-display-ele">
        ...
        <pre class="normal-view" id="csv-output">
        </pre>
        
        <div class="edit-view">
            <div>
                Name: <input class="name-edit-input" value="${user.first_name}">
            </div>
            <div>
                Name: <input class="name-edit-input" value="${user.last_name}">
            </div>
            <div>
...
async function streamUsers() {
    const response = await httpGet('/api/users/stream');
...
async function uploadToS3() {
    return httpPost('/api/users/uploadToS3');
}
